...
UA = "dodo-belgrade-location-scout/1.0 (s.artemov@dodobrands.io)"
MIRRORS = [
...
_KONTUR = None
_KONTUR_PATH = os.path.join(os.environ.get("BG_DATA", "/Users/dodo/pizzeria-location-monitor"), "kontur_belgrade.npz")
...
def score_line(sc):
    ...
    if not sc or sc.get("score") is None:
        return ""
    parts = [f"{score_emoji(sc['score'])} Скоринг {sc['score']}/100"]
    parts.append(f"👥 ~{sc['residents_500']:,} жит. в 500 м".replace(",", " "))
    extra = []
    if sc["transit_300"]:
        extra.append(f"🚏 {sc['transit_300']} ост. в 300 м")
    if sc["offices_500"] or sc["edu_500"]:
        extra.append(f"🏢 {sc['offices_500']} офис./{sc['edu_500']} образ.")
    if sc["competitors_200"]:
        extra.append(f"⚔️ {sc['competitors_200']} конкур. в 200 м")
    if sc.get("nearest_dodo_km") is not None:
        extra.append(f"🍕 Dodo {sc['nearest_dodo_km']} км")
    line = " · ".join(parts)
    if extra:
        line += "\n   " + " · ".join(extra)
    return line
